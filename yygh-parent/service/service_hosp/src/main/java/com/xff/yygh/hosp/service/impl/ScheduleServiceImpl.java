...
import com.xff.yygh.hosp.repository.ScheduleRepository;
...
public  class ScheduleServiceImpl implements ScheduleService {
    ...
    @Autowired
    private MongoTemplate mongoTemplate;  //操作数据库，有的方式方便
    @Autowired
    ...
    public void save(Map<String, Object> paramMap) {
        ...
        //把参数map 集合 转换成对象 schedule
        String mapString = JSONObject.toJSONString(paramMap);
        Schedule schedule = JSONObject.parseObject(mapString, Schedule.class);//string转换为对象
        ...
        if(scheduleExist !=null){
            ...
            scheduleExist.setStatus(1);  // 排班状态（-1：停诊 0：停约 1：可约
            scheduleExist.setIsDeleted(0);
            scheduleRepository.save(scheduleExist);
        }else {
            //做添加     使用 传入的这个 数据对象  schedule
            schedule.setCreateTime(new Date());
            schedule.setUpdateTime(new Date());
            schedule.setStatus(1);  // 排班状态（-1：停诊 0：停约 1：可约
    ...
    public Map<String, Object> getRuleSchedule(Long page, Long limit, String hoscode, String depcode) {
        ...
        //调用方法，最终执行
        AggregationResults<BookingScheduleRuleVo> aggResults =  //传入条件，传入实体类型， 要返回的实体类型
